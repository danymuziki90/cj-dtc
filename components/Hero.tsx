...
import type { HeroSlideData } from '@/lib/hero/types'
...
// ─── Slides statiques par défaut (fallback si pas de données DB) ──────────────
...
// ─── Convertit un HeroSlideData (DB) en HeroSlideItem (legacy) ───────────────
function dbSlideToItem(s: HeroSlideData, index: number): HeroSlideItem {
  return {
    id: index + 1,
...
// ─── Props ────────────────────────────────────────────────────────────────────
interface HeroProps {
  /** Slides dynamiques depuis la DB (optionnel — fallback vers HERO_SLIDES si absent) */
  dynamicSlides?: HeroSlideData[]
}

export default function Hero({ dynamicSlides }: HeroProps) {
  const params = useParams<{ locale?: string }>()
  const locale = params?.locale || 'fr'
  const isFr = locale === 'fr'

  // Utiliser les slides DB si disponibles, sinon fallback statique
  const slides: HeroSlideItem[] =
    dynamicSlides && dynamicSlides.length > 0
      ? dynamicSlides.map(dbSlideToItem)
      : HERO_SLIDES
  ...
  return (
    <section
      className="hero-bg-unified relative min-h-[75vh] pt-32 sm:pt-36 lg:pt-48 xl:pt-52 pb-16 overflow-hidden flex flex-col justify-center"
    ...
    >
      ...
      <div className="absolute inset-0 z-0 select-none overflow-hidden">
        ...
        {/* Gradient overlays */}
        <div className="absolute inset-0 bg-gradient-to-r from-black/60 via-black/35 to-black/15 z-20" />
        <div className="absolute inset-x-0 top-0 h-40 bg-gradient-to-b from-black/45 via-black/15 to-transparent z-20 hidden lg:block pointer-events-none" />
      </div>
