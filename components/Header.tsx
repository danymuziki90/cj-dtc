...
export default function Header() {
  ...
  const studentSpaceHref = `/${locale}/espace-etudiants`
        ...
        <nav className="hidden items-center gap-1 md:flex" aria-label="Navigation principale">
          {links.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="rounded-lg px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-100 hover:text-[var(--cj-blue)]"
            >
              {link.label}
            </Link>
          ))}
          ...
          <Link
            href={`/${locale}/contact`}
            className="inline-flex items-center gap-2 rounded-lg bg-[var(--cj-red)] px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-[var(--cj-red-700)]"
          >
            {labels.contact}
          </Link>
        </nav>
