...
export default function EspaceEtudiantsPage() {
  ...
  async function loadDashboard() {
    ...
    if (response.status === 401 || response.status === 403) {
      setAuthError(
        payload.error ||
          "Vous devez vous connecter pour acceder a l'espace etudiant.",
      );
      setLoading(false);
      return;
    }
      ...
      <div className="flex min-h-screen items-center justify-center bg-[linear-gradient(135deg,#041224_0%,#002d72_45%,#eef5ff_100%)] px-4">
        <div className="w-full max-w-md rounded-[30px] border border-white/20 bg-white/10 p-8 text-white shadow-2xl backdrop-blur-xl">
          ...
          <p className="mt-3 text-sm leading-6 text-white/80">
            Nous preparons vos paiements, sessions et ressources dans une vue
            unique.
          </p>
        ...
        <div className="w-full max-w-2xl overflow-hidden rounded-[32px] border border-white bg-white shadow-[0_30px_90px_-35px_rgba(0,45,114,0.4)]">
          <div className="bg-[linear-gradient(120deg,#001737_0%,#002d72_52%,#0c4da2_100%)] px-8 py-10 text-white">
            ...
            <p className="mt-3 max-w-xl text-sm leading-6 text-white/80">
              Cet espace centralise vos inscriptions, vos travaux et vos
              justificatifs de paiement dans un environnement protege.
            </p>
          </div>
          ...
          <div className="relative grid gap-8 xl:grid-cols-[1.5fr_0.95fr]">
            <div>
              ...
              <div className="mt-5 flex flex-wrap items-center gap-3">
                <h2 className="text-4xl font-semibold tracking-tight leading-tight sm:text-5xl">
                  Pilotage complet de votre parcours
                </h2>
                <span
                  className={`rounded-full border px-3 py-1 text-xs font-semibold ${statusClass(student.status)}`}
                >
                  {student.status}
                </span>
              </div>
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                ...
                {firstCertificate ? (
                  <a
                    href={
                      firstCertificate.fileUrl || `${basePath}/mes-certificats`
                    }
                    className={studentSecondaryButtonClassName}
                  >
                  ...
                  {[
                    {
                      ...
                      done: Boolean(eligibility.paymentValidated),
                    },
                    {
                      label: "Projet valide",
                      done: Boolean(eligibility.projectValidated),
                    },
                    {
                      label:
                        eligibility.attendanceTracked &&
                    ...
                    <div
                      key={item.label}
                      className="flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-slate-950/20 px-4 py-3 text-sm text-white/90"
                    >
          ...
          <div className="space-y-6">
            ...
            >
              <div className="grid gap-4 lg:grid-cols-[1.05fr_1fr]">
                <div
                  className={`${subtleCardClassName} bg-[linear-gradient(180deg,#f8fbff_0%,#eef5ff_100%)]`}
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="min-w-0">
                      ...
                      <span
                        className={`mt-3 inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${statusClass(student.status)}`}
                      >
                        {student.status}
                      </span>
                    </div>
                ...
                <div className={subtleCardClassName}>
                  <div className="flex items-center justify-between gap-3">
                    ...
                    {currentSession ? (
                      <span
                        className={`rounded-full border px-3 py-1 text-xs font-semibold ${statusClass(currentSession.lifecycle)}`}
                      >
                  ...
                  {currentSession ? (
                    <div className="mt-5 grid gap-3 sm:grid-cols-2">
                      ...
                      <div className="rounded-2xl border border-slate-200 bg-white p-4">
                        ...
                        <p className="mt-2 text-sm font-medium text-slate-900">
                          {[currentSession.sessionType, currentSession.format]
                            .filter(Boolean)
                            .join(" | ") || "-"}
                        </p>
                      </div>
                      <div className="rounded-2xl border border-slate-200 bg-white p-4">
                        ...
                        <p className="mt-2 text-sm font-medium text-slate-900">
                          {currentSession.availableSpots ?? "-"} places libres |
                          Reservation {currentSession.reservedSpot ?? "-"}
                        </p>
                      </div>
                    </div>
                  ) : (
            ...
            >
              <div className="grid gap-4 xl:grid-cols-[0.95fr_1.15fr]">
                <form
                  onSubmit={submitWork}
                  className="rounded-3xl border border-blue-100 bg-[linear-gradient(180deg,#f8fbff_0%,#eef5ff_100%)] p-5"
                >
                  <p className="text-sm font-semibold text-slate-950">
                    Nouvelle soumission
                  </p>
                  <p className="mt-1 text-sm text-slate-500">
                    Chargez un livrable en PDF ou image avec un titre clair.
                  </p>
                  {submissionError ? (
                    <p className="mt-4 rounded-2xl bg-red-50 px-4 py-3 text-sm text-red-600">
                      {submissionError}
                    ...
                    submissions.slice(0, 5).map((submission: any) => (
                      ...
                      >
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          ...
                          <span
                            className={`rounded-full border px-3 py-1 text-xs font-semibold ${statusClass(submission.status)}`}
                          >
                            {submission.status}
                          </span>
                        </div>
                        {submission.reviewFeedback ? (
                          <div className="mt-4 rounded-2xl border border-blue-100 bg-blue-50/70 px-4 py-3 text-sm text-slate-700">
                            <span className="font-semibold text-[var(--cj-blue)]">
                              Feedback admin:
                            </span>{" "}
                            {submission.reviewFeedback}
                          </div>
                  ...
                  {[
                    {
                      ...
                      value: progress.hoursCompleted ?? 0,
                    },
                    {
                      label: "Heures restantes",
                      value: progress.hoursRemaining ?? 0,
                    },
                    {
                      label: "Exercices termines",
                      value: progress.exercisesCompleted ?? 0,
                    },
                    {
                      label: "Exercices en cours",
                      value: progress.exercisesInProgress ?? 0,
                    },
                    {
                      label: "Projets termines",
                      value: progress.projectsCompleted ?? 0,
                    },
                    {
                      label: "Evaluations",
                      value: progress.evaluationsCompleted ?? 0,
                    },
                  ].map((item) => (
                    <div
                      key={item.label}
                  ...
                  notifications.map((item: any) => (
                    ...
                    >
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        ...
                        <span
                          className={`rounded-full border px-3 py-1 text-xs font-semibold ${statusClass(item.type)}`}
                        >
                          {item.type}
                        </span>
                      </div>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="mt-2 text-xs text-slate-500">
                            Ajoute le {formatDate(resource.createdAt)}
                          </p>
                        </div>
                      </div>
                    </a>
              ...
              sessionsHistory.map((item: any) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span
                      className={`rounded-full border px-3 py-1 text-xs font-semibold ${statusClass(item.paymentStatus)}`}
                    >
                      {item.paymentStatus}
                    </span>
                  </div>
                  <div className="mt-4 space-y-2 text-sm text-slate-600">
                    ...
                    <p>
                      ...
                      {lifecycleLabel(item.sessionLifecycle)}
                    </p>
                    <p>
                      <span className="font-semibold text-slate-900">
                        Heures:
