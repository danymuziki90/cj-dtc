    ...
    <div className="relative min-h-screen overflow-hidden bg-slate-950 text-slate-800">
      ...
      <main className="relative z-10 mx-auto max-w-7xl space-y-8 px-4 pb-16 pt-6 sm:px-6 lg:px-8">
        ...
        <div className="space-y-8">
          ...
          {/* TAB 3: TRAVAUX (ASSIGNMENTS) */}
          {activeTab === "travaux" && (
            <div className="space-y-6">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  <h3 className="text-lg font-bold text-slate-900">Mes Travaux & Évaluations</h3>
                  <p className="text-xs text-slate-500">Consultez vos TP, projets, examens et remettez vos devoirs en ligne.</p>
                </div>
                <Link
                  href={`${basePath}/travaux`}
              ...
              {assignments.length === 0 ? (
                <div className="rounded-3xl border border-slate-200 bg-white p-12 text-center text-xs text-slate-500 space-y-3">
                  ...
                  <p className="font-bold text-slate-800 text-sm">Aucun devoir à réaliser pour le moment</p>
                  <p className="text-slate-500">Vos prochains travaux apparaîtront automatiquement dès leur publication.</p>
                </div>
              ) : (
                <div className="grid gap-4 md:grid-cols-2">
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          <div className="space-y-1">
                            ...
                            <h4 className="text-sm font-bold text-slate-900 leading-snug line-clamp-1">{assign.title}</h4>
                            <p className="text-[11px] text-slate-500 font-semibold">{assign.formation?.title || "Formation"}</p>
                          </div>
                          <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-2.5 py-1 rounded-full border ${statusInfo.color}`}>
