...
export async function GET(request: NextRequest) {
  ...
  const [enrollmentsRaw, portalCertificates, issuedCertificates, news, evaluations, assignments] =
    await Promise.all([
      ...
      prisma.assignment.findMany({
        ...
        include: {
          Formation: { select: { title: true, titleEn: true } },
          TrainingSession: { select: { id: true, location: true, locationEn: true } },
          AssignmentFile: true,
          Submission: {
            where: { studentId: auth.student.id },
  ...
  const availableSessions = openSessionsRaw
    ...
    .map((s) => {
      ...
      return {
        ...
        formationTitleEn: (s as any).formation?.titleEn || null,
        ...
        endDate: s.endDate,
        location: s.location,
        locationEn: (s as any).locationEn || null,
        format: s.format,
        formatEn: (s as any).formatEn || null,
        status: s.status,
  ...
  const sessionsHistory = enrollmentsWithSession.map((item) => {
    ...
    return {
      ...
      formationTitleEn: (item.formation as any).titleEn || null,
      ...
      formationDescriptionEn: (item.formation as any).descriptionEn || null,
      sessionId: session.id,
      sessionType: sessionMeta.metadata.sessionType || null,
      startDate: session.startDate,
      endDate: session.endDate,
      location: session.location,
      locationEn: (session as any).locationEn || null,
      format: session.format,
      formatEn: (session as any).formatEn || null,
