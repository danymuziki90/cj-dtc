...
function WidgetError({ message, onRetry }: { message: string; onRetry: () => void }) {
  return (
    <div className="flex items-start gap-4 rounded-[26px] border border-rose-200 bg-rose-50 dark:bg-rose-950/20 px-5 py-4 w-full h-full">
      <AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-rose-600 dark:text-rose-450" />
      <div>
        <p className="font-semibold text-rose-900 dark:text-rose-200">Données indisponibles</p>
        <p className="mt-1 text-sm text-rose-700 dark:text-rose-350">{message}</p>
        <button
          onClick={onRetry}
...
export default function AdminDashboardPage() {
  ...
  const [errorReporting, setErrorReporting] = useState<string | null>(null)
  ...
  const fetchReporting = async () => {
    ...
    try {
      const res = await fetch('/api/admin/system/reporting?period=30d', { cache: 'no-store' })
      if (!res.ok) throw new Error('API reporting failed')
      setReportingData(await res.json())
    } catch (err: any) {
      setErrorReporting(err.message || "Erreur lors du chargement du reporting.")
    } finally {
  ...
  const fetchKpi = async () => {
    ...
    setErrorKpi(null)
    try {
      const res = await fetch('/api/admin/dashboard/kpi', { cache: 'no-store' })
      if (!res.ok) throw new Error('API kpi failed')
      setKpiData(await res.json())
    } catch (err: any) {
      setErrorKpi(err.message || "Erreur lors du chargement des KPIs.")
    } finally {
      setLoadingKpi(false)
    }
  }
  ...
  const priorityCount = useMemo(() => {
    ...
    return (
      ...
      (reportingData.actionsNow?.certificatesToIssue?.length || 0)
    )
  }, [reportingData])

  const pedagogicalAlerts = useMemo(() => {
    if (!reportingData?.alerts) return []
    return reportingData.alerts.filter(
      (alert) =>
  ...
  const quickModuleStats = useMemo(() => {
    return [
  ...
  const isGlobalLoading = loadingReporting && loadingStats && loadingKpi

  if (isGlobalLoading) {
    return (
      <AdminShell title="Pilotage">
        <div className="space-y-6 animate-pulse">
          <div className="h-44 w-full rounded-[30px] bg-slate-200 dark:bg-slate-800" />
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {[...Array(8)].map((_, i) => (
              <div key={i} className="h-32 rounded-[26px] bg-slate-200 dark:bg-slate-800" />
            ))}
          </div>
          <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
            <div className="h-96 rounded-[30px] bg-slate-200 dark:bg-slate-800" />
            <div className="h-96 rounded-[30px] bg-slate-200 dark:bg-slate-800" />
          </div>
        </div>
      </AdminShell>
  ...
  return (
    <AdminShell title="Pilotage">
      <div className="space-y-6 animate-fade-in-up">
        ...
        {!loadingReporting && pedagogicalAlerts.length > 0 && (
        ...
        {errorReporting && !reportingData && pedagogicalAlerts.length === 0 && (
          <WidgetError message="Impossible de charger les alertes pédagogiques." onRetry={fetchReporting} />
        )}

        {errorKpi || errorStats ? (
          <WidgetError 
            message="Impossible de charger toutes les métriques clés." 
            onRetry={() => { if(errorKpi) fetchKpi(); if(errorStats) fetchStats(); }} 
          />
        ) : loadingKpi || loadingStats ? (
          <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 animate-pulse">
            {[...Array(8)].map((_, i) => (
              <div key={i} className="h-32 rounded-[26px] bg-slate-200 dark:bg-slate-800" />
            ))}
          </div>
          ...
          <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
            <DashboardKpiCard
              ...
              value={kpiData.totals?.studentsTotal || 0}
              helper="Total des comptes enregistrés"
              trend={kpiData.trends?.studentTrend || '0%'}
              trendDirection="up"
              tone="primary"
              href="/admin/students"
            />
            <DashboardKpiCard
              ...
              value={kpiData.totals?.studentsActive || 0}
            ...
            <DashboardKpiCard
              ...
              value={kpiData.totals?.sessionsOpen || 0}
              helper="Cohortes de formation actives"
              tone="primary"
              href="/admin/sessions"
            />
            <DashboardKpiCard
              ...
              value={kpiData.totals?.sessionsFuture || 0}
            ...
            <DashboardKpiCard
              icon={FolderKanban}
              label="Travaux remis"
              value={(statsData.totalInscriptions || 0) - (statsData.pendingCorrections || 0)}
            ...
            <DashboardKpiCard
              ...
              value={kpiData.totals?.newsPublished || 0}
            ...
            <DashboardKpiCard
              icon={BellRing}
              label="Notifications"
              value={kpiData.totals?.notificationsTotal || 0}
        ...
        <div className="grid gap-6 xl:grid-cols-[1.25fr_0.75fr]">
          <AdminPanel>
            ...
            />
            {errorStats ? (
              <div className="mt-6"><WidgetError message="Erreur de chargement de la tendance des inscriptions." onRetry={fetchStats} /></div>
            ) : loadingStats || !statsData ? (
              <div className="mt-6 h-64 sm:h-72 md:h-80 w-full animate-pulse rounded-2xl bg-slate-200 dark:bg-slate-800" />
            ) : (
          ...
          <AdminPanel>
            ...
            />
            {errorReporting ? (
              <div className="mt-6"><WidgetError message="Erreur de chargement du taux de conversion." onRetry={fetchReporting} /></div>
            ) : loadingReporting || !reportingData ? (
              <div className="mt-6 h-64 sm:h-72 md:h-80 w-full animate-pulse rounded-2xl bg-slate-200 dark:bg-slate-800" />
            ) : (
              <div className="mt-6 flex flex-col items-center justify-center text-center">
                <div className="relative inline-flex items-center justify-center">
                  <span className="text-4xl font-black text-slate-905">{reportingData.summary?.accountConversionRate || 0}%</span>
                </div>
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          <AdminPanel>
            ...
            />
            {errorStats ? (
              <div className="mt-6"><WidgetError message="Erreur de chargement des activités." onRetry={fetchStats} /></div>
            ) : loadingStats || !statsData ? (
              <div className="mt-6 h-64 w-full animate-pulse rounded-2xl bg-slate-200 dark:bg-slate-800" />
            ) : (
          ...
          <AdminPanel>
            ...
            />
            {errorReporting ? (
              <div className="mt-6"><WidgetError message="Erreur de chargement des travaux soumis." onRetry={fetchReporting} /></div>
            ) : loadingReporting || !reportingData ? (
              <div className="mt-6 h-64 w-full animate-pulse rounded-2xl bg-slate-200 dark:bg-slate-800" />
            ) : (
              <div className="mt-6 space-y-2 max-h-[360px] overflow-y-auto pr-1">
                ...
                {(!reportingData.actionsNow?.submissionsPendingReview || reportingData.actionsNow.submissionsPendingReview.length === 0) && (
          ...
          <AdminPanel>
            ...
            />
            {errorReporting ? (
              <div className="mt-6"><WidgetError message="Erreur de chargement des dossiers en attente." onRetry={fetchReporting} /></div>
            ) : loadingReporting || !reportingData ? (
              <div className="mt-6 h-64 w-full animate-pulse rounded-2xl bg-slate-200 dark:bg-slate-800" />
            ) : (
              <div className="mt-6 space-y-2 max-h-[360px] overflow-y-auto pr-1">
                ...
                {(!reportingData.actionsNow?.studentsBlockedWithoutAccount || reportingData.actionsNow.studentsBlockedWithoutAccount.length === 0) && (
