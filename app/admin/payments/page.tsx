...
export default function PaymentsPage() {
  ...
  async function loadReferenceData() {
    try {
      ...
      console.error('Unable to load payment reference data:', error)
      setSessions([])
      setEnrollments([])
    }
  }
      ...
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-600">
              Recherche, filtres et pagination serveur pour les transactions etudiants.
            </p>
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4">
            ...
            <p className="mt-1 text-2xl font-semibold text-slate-900">{formatCurrency(summary.successfulAmount)}</p>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white p-4">
            <p className="text-sm text-slate-500">Paiements en attente</p>
            <p className="mt-1 text-2xl font-semibold text-red-700">{summary.pendingCount}</p>
          </div>
        </div>

        {showCreateForm ? (
          <form onSubmit={createManualPayment} className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="grid gap-3 rounded-xl border border-slate-200 bg-white p-4 md:grid-cols-4">
          <div>
            ...
            <input
              ...
              placeholder="Etudiant, email, reference"
              className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"
            />
          </div>
          <div>
