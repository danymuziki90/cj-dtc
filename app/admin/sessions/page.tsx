...
export default function AdminSessionsPage() {
  ...
  const [docForm, setDocForm]                   = useState({ title: '', description: '', category: 'pdf' })
  ...
  useEffect(() => {
    if (!editingSession) {
      setSessionDocuments([])
      setDocFile(null)
      setDocForm({ title: '', description: '', category: 'pdf' })
      setDocError(null)
      return
    }
    ...
    async function loadDocuments() {
      ...
      try {
        ...
        console.error("Erreur lors du chargement des documents de la session :", err)
      } finally {
        setDocLoading(false)
      }
    }
  ...
  const handleDocUpload = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!docFile || !editingSession) return
    ...
    try {
      ...
      formDataUpload.append('description', docForm.description)
      formDataUpload.append('category', docForm.category)
      formDataUpload.append('sessionId', String(editingSession.id))
      formDataUpload.append('isPublic', 'true') // Always public for students
      ...
      if (response.ok) {
        ...
        setSessionDocuments(prev => [newDoc, ...prev])
        setDocFile(null)
        setDocForm({ title: '', description: '', category: 'pdf' })
        const input = document.getElementById('session-doc-file-input') as HTMLInputElement
        if (input) input.value = ''
      } else {
        const errData = await response.json().catch(() => ({}))
        setDocError(errData.error || "Erreur lors du téléversement du document")
      }
    } catch (err: any) {
      console.error(err)
      setDocError(err.message || "Erreur réseau lors du téléversement")
    } finally {
  ...
  const handleDocDelete = async (docId: number) => {
    if (!confirm("Voulez-vous supprimer ce support pédagogique ?")) return
    try {
      ...
      if (response.ok) {
        ...
        const errData = await response.json().catch(() => ({}))
        alert(errData.error || "Impossible de supprimer le document")
      }
    } catch (err) {
      console.error(err)
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl w-full max-w-3xl p-6 relative max-h-[92vh] overflow-y-auto animate-fade-in-up my-4">
            ...
            <form onSubmit={handleFormSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              {/* Supports Pédagogiques (uniquement en édition) */}
              {editingSession && (
                <div className="md:col-span-2 border-t border-slate-100 pt-6">
                  <div className="mb-4">
                    <h3 className="text-sm font-black text-slate-900">📚 Supports pédagogiques de la session</h3>
                    <p className="text-[10px] text-slate-400 mt-0.5">Ajoutez et gérez les documents de cours téléchargeables pour cette session.</p>
                  </div>

                  {/* Formulaire d'ajout rapide */}
                  <div className="p-4 bg-slate-50/60 rounded-2xl border border-slate-200/60 mb-4 space-y-3">
                    <p className="text-[11px] font-extrabold text-slate-700">Ajouter un nouveau support</p>
                    
                    {docError && (
                      <p className="text-[10px] text-red-650 font-bold bg-red-50 border border-red-100 rounded-lg p-2">{docError}</p>
                    )}

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        <label className="block text-[9px] font-bold uppercase tracking-wider text-slate-450 mb-1">Fichier *</label>
                        <input
                          id="session-doc-file-input"
                          type="file"
                          required
                          onChange={(e) => {
                            const file = e.target.files?.[0]
                            if (file) {
                              setDocFile(file)
                              if (!docForm.title) {
                                setDocForm(prev => ({ ...prev, title: file.name.split('.').slice(0, -1).join('.') }))
                              }
                            }
                          }}
                          className="w-full text-xs font-semibold text-slate-600 file:mr-2 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-[10px] file:font-bold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 cursor-pointer"
                        />
                      </div>

                      <div>
                        <label className="block text-[9px] font-bold uppercase tracking-wider text-slate-450 mb-1">Type de ressource *</label>
                        <select
                          value={docForm.category}
                          onChange={(e) => setDocForm(prev => ({ ...prev, category: e.target.value }))}
                          className="w-full px-3 py-1.5 text-xs border border-slate-200 bg-white rounded-lg focus:outline-none font-bold text-slate-700"
                        >
                      ...
                      <div className="sm:col-span-2">
                        <label className="block text-[9px] font-bold uppercase tracking-wider text-slate-450 mb-1">Titre du support *</label>
                        <input
                          type="text"
                          required
                          value={docForm.title}
                          ...
                          className="w-full px-3 py-1.5 text-xs border border-slate-200 bg-white rounded-lg focus:outline-none font-semibold text-slate-750"
                        />
                      </div>

                      <div className="sm:col-span-2">
                        <label className="block text-[9px] font-bold uppercase tracking-wider text-slate-450 mb-1">Description (optionnelle)</label>
                        <input
                          type="text"
                          value={docForm.description}
                          onChange={(e) => setDocForm(prev => ({ ...prev, description: e.target.value }))}
                          placeholder="ex. Slides complémentaires et exercices pratiques de la première journée"
                          className="w-full px-3 py-1.5 text-xs border border-slate-200 bg-white rounded-lg focus:outline-none font-semibold text-slate-700"
                        />
                      </div>
                    </div>
                    ...
                    <div className="flex justify-end pt-2">
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-200 disabled:text-slate-400 text-white text-xs font-bold rounded-xl shadow-sm transition"
                      >
                        {docUploading ? (
                  ...
                  {/* Liste des supports pédagogiques existants */}
                  {docLoading ? (
                    ...
                    <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50/50 py-6 text-center text-xs text-slate-400">
                      Aucun support pédagogique lié à cette session.
                    </div>
                  ) : (
                    <div className="space-y-2 max-h-56 overflow-y-auto pr-1 font-sans">
                      {sessionDocuments.map((doc) => {
                        ...
                        return (
                          <div key={doc.id} className="flex items-center justify-between p-3 bg-white border border-slate-200/70 rounded-xl shadow-sm">
                            <div className="min-w-0 pr-3">
                              <div className="flex items-center gap-2">
                                <span className="px-2 py-0.5 bg-slate-100 border border-slate-200 rounded-lg text-[9px] font-bold text-slate-500 uppercase">
                                  {fileLabels[doc.category] || doc.category}
                                </span>
                                <p className="font-extrabold text-slate-800 text-xs truncate">{doc.title}</p>
                              </div>
                              {doc.description && (
                                <p className="text-[10px] text-slate-450 mt-1 font-semibold truncate">{doc.description}</p>
                              )}
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              ...
                              <button
                                ...
                                className="p-1.5 hover:bg-red-50 border border-slate-200 hover:border-red-200 rounded-lg text-slate-400 hover:text-red-650 transition"
                                title="Supprimer"
                              >
