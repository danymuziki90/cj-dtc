...
export default function StudentSessionsPage() {
  ...
  if (loading) {
    return (
      <StudentPortalLoading
        title="Mes sessions"
        description="Suivez vos reservations, vos prochaines dates et votre historique."
        icon={GraduationCap}
      />
    );
  }
          ...
          <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
            <div className="rounded-3xl border border-slate-200 bg-slate-50/80 p-5">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[var(--cj-red)]">
                    {sessionTypeLabel(currentSession.sessionType)}
                  </p>
                ...
                <span
                  className={`rounded-full border px-3 py-1 text-xs font-semibold ${statusToneClass(currentSession.status)}`}
                >
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2">
                <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3">
                  ...
                  <p className="mt-1 text-sm font-semibold text-slate-900">
                    {formatPortalDateTime(currentSession.startDate)}
                  </p>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3">
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">
                  ...
                  <p className="mt-1 text-sm font-semibold text-slate-900">
                    {formatPortalDateTime(currentSession.endDate)}
                  </p>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3">
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">
                ...
                <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3">
                  ...
                  <p className="mt-1 text-sm font-semibold text-slate-900">
                    {currentSession.sessionStatus || "En cours"}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-5">
              ...
              <div className="mt-4 space-y-3 text-sm text-slate-600">
                ...
                <div className="flex items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                  ...
                  <strong className="text-slate-950">
                    {currentSession.currentParticipants ?? 0} /{" "}
                    {currentSession.maxParticipants ?? 0}
                  </strong>
                </div>
                <div className="flex items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                  <span>Places disponibles</span>
                  <strong className="text-slate-950">
                    {currentSession.availableSpots ?? 0}
                  </strong>
                </div>
                <div className="flex items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                  <span>Etat</span>
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-500">
                      {session.formationCategory || "Formation"}
                    </p>
