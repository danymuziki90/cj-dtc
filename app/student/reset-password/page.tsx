...
export default function StudentResetPasswordPage() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      if (!response.ok) {
        setError(data.error || 'Échec de réinitialisation.')
        return
      }
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-100 px-4 py-8">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <h1 className="mb-2 text-2xl font-bold text-slate-900">Réinitialiser le mot de passe</h1>
        <p className="mb-6 text-sm text-slate-600">Définissez un nouveau mot de passe pour votre compte étudiant.</p>
